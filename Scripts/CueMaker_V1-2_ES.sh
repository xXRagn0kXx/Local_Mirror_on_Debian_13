...
Menu_Principal() {
    while true; do
        opcion=$(dialog --backtitle "Generador de CUE para ArkOS" \
        ...
        case $opcion in
            1)
                Modo="MODE2/2352"
                echo "Seleccionaste MODE2/2352" >/dev/tty1
            ...
            2)
                Modo="MODE1/2352"
                echo "Seleccionaste MODE1/2352" >/dev/tty1
                if Seleccionar_Archivo "/roms"; then
                    Cue_Maker "$Modo" "$Ruta_Final"
                fi
                ;;
            3)
                Modo="MODE1/2048"
                echo "Seleccionaste MODE1/2048" >/dev/tty1
                if Seleccionar_Archivo "/roms"; then
                    Cue_Maker "$Modo" "$Ruta_Final"
                fi
                ;; 
            4)
                Modo="MODE2/2336"
                echo "Seleccionaste MODE2/2336" >/dev/tty1
                if Seleccionar_Archivo "/roms"; then
                    Cue_Maker "$Modo" "$Ruta_Final"
                fi
                ;;
            5)
                Modo="AUDIO"
                echo "Seleccionaste AUDIO" >/dev/tty1
                if Seleccionar_Archivo "/roms"; then
                    Cue_Maker "$Modo" "$Ruta_Final"
                fi
                ;;
            *)
                ...
                echo "Saliendo del script. ¡Adios!" >/dev/tty1
                break
                ;;
        esac
...
Seleccionar_Archivo() {
    ...
    while true; do
        ...
        selection=$(dialog \
            --clear \
            --backtitle "Generador de CUE para ArkOS" \
        ...
        case $rc in
            ...
            1)
                ...
                Ruta_Final=""
                dialog --msgbox "No se ha seleccionado ninguna ruta/archivo" 8 60 2>/dev/tty1
                return 1
                ;;
            255)
...
Cue_Maker() {
    ...
    if [[ -d "$Ruta_Final" ]]; then
        dialog --clear
        dialog --title "Advertencia" --yesno "Se van a reemplazar todos los .cue existentes en el directorio: $Ruta_Final \n\nNOTA: Esto llevara un tiempo dependiendo los ficheros a generar y requiere carga para consola, al aceptar no tocar nada hasta que esta pantalla se actualice sola.\n\n¿Desea continuar?" 13 60 2>/dev/tty1
        local rc=$?
        ...
        case $rc in
            0)
                ...
                local total_files=${#bin_files[@]} # Obtenemos el numero total de archivos
                
                if [[ $total_files -gt 0 ]]; then
                    
                    # --- INICIO: Bloque de barra de progreso ---
                    local current_file_num=0
                    {
                        for bin_file in "${bin_files[@]}"; do
                            ...
                            # --- Logica de creacion de archivo (la misma que tenias) ---
                            ...
                            # --- Fin de logica de creacion ---
                            
                            # Enviar actualizacion a dialog
                            # Usamos XXX como delimitador estandar para dialog
                            echo "XXX"
                            echo "$percent"
                            # Este es el texto que pediste:
                            echo "Creando fichero $current_file_num de $total_files"
                            # Añadimos el nombre del fichero actual para mas info
                            echo "$cue_name"
                            echo "XXX"
                            
                        done
                    } | dialog --clear --title "Procesando Directorio" --gauge "Por favor, espere..." 10 70 0 2>/dev/tty1
                    # --- FIN: Bloque de barra de progreso ---

                    # Mensaje de finalizacion
                    dialog --clear
                    dialog --msgbox "Se han generado $total_files archivos .cue en formato $Modo en el directorio: $Ruta_Final" 10 60 2>/dev/tty1
                
                else
                    dialog --clear
                    dialog --msgbox "No se encontraron archivos .bin en el directorio: $Ruta_Final" 10 60 2>/dev/tty1
                fi
                ;;
            1)
    ...
    # If the path is a file
        ...
        if [[ "$Ruta_Final" == *.bin ]]; then
            ...
            dialog --clear
            dialog --msgbox "Se ha generado el fichero: $cue_path" 10 60 2>/dev/tty1
        else
            dialog --clear
            dialog --msgbox "El archivo seleccionado no es un .bin." 10 60 2>/dev/tty1
